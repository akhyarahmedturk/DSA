...
*    Created: 2025-12-27 01:16 (GMT+5)
...
void dfs2(int idx,int p,int curr,vector<vi> &graph,vi &dp){
    //curr jo uper sy mila
    dp[idx]=max(dp[idx],curr)+1;//nechy+uper
    vi ya={curr,0};
    for(auto it:graph[idx]){
        if(it==p) continue;
        ya.pb(dp[it]+1);
    ...
    for(auto it:graph[idx]){
        if(it==p) continue;
        if(dp[it]+1==ya[0]) dfs2(it,idx,ya[1]+1,graph,dp);
